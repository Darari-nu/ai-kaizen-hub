...
const NOTES = {
  ...
  '011-dare-ga-itta': { status: 'fix', note: '🎨路線OKをもらい量産→挿入完了: 図解3枚をこの記事に入れました(①部長→廃止/本部長→継続の対比 ②スマホとAIの「遊んでるのか→仕事に必須」対比 ③頭の中はAIが読めない→テキストに残せば読める)。この検品室で下にスクロールすると全部見られます。本文は8/12検品OKのまま変えていません。図解込みでOKなら✅ください。' },
  ...
  '015-joushisu-shitsumon': { status: 'fix', note: '🎨路線OKをもらい図解3枚を挿入しました(①5系統→3つの守りの対応図 ②答え方3つの型 ③情シスの本音「誰が面倒を見るの？」と先出しする3つの約束)。この検品室で見られます。テキストは8/12反映版のまま。確認1点(継続): 「とある会社」の書き方がうちの会社の話と読まれないか目視ください。' },
  ...
  '021-level-kitei': { status: 'fix', note: '⚙️8/12深夜・機密の件に回答いただき反映: 「ClaudeとChatGPTエンタープライズはマル秘までOK」→判断表を再改定しました。機密は区分A(学習に使われない会社契約AI)=可・区分B(学習保証なし)=不可に。承認が要るのは大量一括処理だけになり、不可は個人情報レベル4・5(+区分Bの機密)のみ。記事021・基準ページ・情セ基の要約もすべて同期済み。確認1点: 記事の推しフレーズ「不可を絞るほど、規程は強くなる」の言い切りでいいですか。' },
