...
const NOTES = {
  ...
  '006-meishi-bot': { status: 'fix', note: '8/8反映: 「いつもの」削除、完成形LINEスクショ+AIブラウザの一文とスクショを記事内に配置しました。⚠️安全のため2箇所加工しています: ①LINEスクショはスプレッドシートの実URLが写っていたので、その手前でカット ②ブラウザのスクショは左サイドバーにDara系リポ名が写っていたので(レーン分離)、サイドバーごとカット。仕上がりを見て、問題なければOKください。⚙️8/10: 規程体系の整備に伴い、レベル表の参照を「AI利用規程・第3条」→「AI入力情報基準」(新ページ)に更新しました。⚙️8/10検品反映: 頭のAIを2択に書き直しました(①Gemini API=無料で始める ②GPT-5.6 Luna=体感こっちが良い、はそのまま使用)。Lunaは公式リンク付きで$0.20/$1.20per100万トークン→名刺1枚≈0.03円・1,000枚で30円まで計算して記載。' },
  '007-rpa-daitai': { status: 'fix', note: '8/8反映: ①RPAのいいところ(プロUIと素人UIの差/AIで縮む)を追加 ②ダラリ重工業の会議録発行システムを実際に作りました(/company/intranet/kaigiroku/ で遊べます)。AIが自動入力→発行番号取得した実スクショ2枚を記事に掲載。生成画像でアプリ画面を偽造するのは誤解を招くのでやめ、実際に動くもので撮る方式にしました。実物の雰囲気と離れすぎていないか見てください。⚙️8/10検品反映: 図解つきスクショ(①AIが埋めた②発行もAIが押す③番号取得まで約20秒の3注釈)と、実機で自動入力→発行番号取得までの10秒動画(mp4)を追加しました。' },
  '008-shanai-system': { status: 'fix', note: '8/8反映: 分厚いマニュアルを実際に作りました(/company/intranet/kaigiroku-manual/ 第7版・全87ページの体。改訂履歴7回中3回が誤字修正)。記事から実物にリンクし、読ませるコツ3つを追加: ①先にAIに「1トピック1ブロック」に変換させる(ツェッテルカステン式) ②ChatGPT/Claudeならプロジェクト、Claude Code/Codexならスキル化で毎回渡さない ③出典(章番号)を言わせて更新漏れに気づく。⚙️8/10検品反映: やること3ステップの図解スクショ+マニュアル表紙(第7版・全87ページ・改訂履歴)のスクショ+全ページスクロール動画を追加。「AIに聞いている画面」の実チャットスクショは画面偽造禁止の裁定に従い入れていません。darariの実機スクショが1枚あると完璧です。' },
  ...
  '014-nyuuryoku-handan': { status: 'new', note: '【新規・ガバナンス】002の実践編。抽象化テクが核。確認したいこと3つ: ①ビフォー例文「ダラリ精工さま向けの減速機DR-2200・第3工場の鋳造ライン」は架空の作例です。実在情報と紛らわしくないですか ②「慣れると10秒」「立ち話テスト」は説明用の言い回しで実測ではありません ③失敗談の締め「いまのぼくなら止める側です」(前職のメール転送を正規ルート外と自己評価)、この強さでOKか⚙️8/10: 規程体系の整備に伴い、レベル表の参照を「AI利用規程・第3条」→「AI入力情報基準」(新ページ)に更新しました。' },
  ...
  '017-gijiroku': { status: 'new', note: '【新規・AIカイゼン】議事録を「清書する仕事」から「確認する仕事」へ。確認したいこと3つ: ①失敗談の「会議中にPCでメモ→内職と注意された」に「あとでAIに渡せて合理的だと思っていた」という動機を足しています。記憶と合っていますか ②Q1で「議事メモはレベル2、取引先情報や原価が乗るとレベル3」と規程を引いています。設定と矛盾ないですか ③Q3の「読まずにハンコを押すのと同じ」の比喩(ハンコ頭の社長は以前却下したので、気になるなら差し替えます)⚙️8/10: 規程体系の整備に伴い、レベル表の参照を「AI利用規程・第3条」→「AI入力情報基準」(新ページ)に更新しました。' },
  ...
  '020-hp-seisaku': { status: 'new', note: '【新規・記録】このサイト自体をAIと作った3日間のメタ実況(gitログが一次資料)。「いま読んでいるこのサイトが実物です」構造。確認3点: ①「ぼくは1行もコードを書いていません」と断定しました(コミット履歴からの判断)。手で触った瞬間があるなら弱めます ②架空会社を「調子に乗って作った→我に返って止めた」の内心はgit時刻からの再構成です ③「仕事の外の時間で作りました」はコミット時刻(早朝・夜中心)からの推測。会社特定リスクにも関わるので要確認。' },
