...
const NOTES = {
  ...
  '006-meishi-bot': { status: 'fix', note: '🎨8/13検品反映: 「しくみは目と頭の分業です」の下に5ステップの流れ図解を挿入しました(①LINEで名刺を送る→②目=Vision APIが文字を読む→③頭=AIが仕分け→④スプレッドシートに1行→⑤LINEに完了返信)。本文になかったステップ⑤の1文だけ加筆し、図解と本文を一致させています。この検品室で見られます。(8/12反映済みの料金修正も継続掲載中。ChatGPT側の料金だけお手すきに公式で目視ください)' },
  '007-rpa-daitai': { status: 'ok', note: '✅8/13検品OK「修正なし」。ありがとうございます！' },
  '008-shanai-system': { status: 'fix', note: '⚙️8/13検品「内容は非常に良いが読みにくい」→内容・実話・スクショ・動画・リンクは一切変えず、読みやすさだけ改稿——長い段落を1〜2文単位に分割、括弧に畳んでいた説明を地の文に、3つ以上の列挙は箇条書きに、見出し直後に要旨の1文。唯一の加筆は「発展形」節頭の要旨文(AIは手順を聞く相手から手順書を作る係になります)だけです。' },
  '011-dare-ga-itta': { status: 'ok', note: '✅8/13検品OK「修正なし」(図解3枚込み)。ありがとうございます！' },
  '012-kuni-mo-yare': { status: 'fix', note: '⚙️8/13検品反映: ポジション取りの導入に、2000年前後のパソコン普及期の具体例を追加しました(「あの人に聞けば分かる」で仕事が集まった→いまのAIはあのときのパソコンと同じ位置)。世間一般の話として記述、本人体験にはしていません。確認継続1点: タイトル末尾「だから、メールを1本だけ」を残すか、「詳しい人の椅子を取っておく」系に変えるか。' },

  // --- 8/8 新規5本（Fableサブエージェントが1本ずつ執筆） ---
  '014-nyuuryoku-handan': { status: 'fix', note: '⚙️8/13検品反映: ①掛け合わせの説明を再構成——結論1行(入れていいかは「何を」×「どのAIに」で決まる。それだけです)→軸を1本ずつ→議事メモ×会社契約AI/個人無料AIの2行対比表、の順に ②抽象化の説得力を補強——「AIが使うのは固有名詞でなく相談の骨組み。AIが持つのは一般知識で、社名からは何も引き出せない」+Before/Afterをもう1組(保守契約の値上げ交渉・架空作例と明示)+「抽象化が効かない質問」(エラーコード・契約条項→会社契約AIの出番)の正直な線引き。確認1点: 追加作例の題材(ダラリ物流さまとの保守契約の値上げ交渉)でよいか。' },
  '015-joushisu-shitsumon': { status: 'fix', note: '⚙️8/13検品反映: ①型①に前提を追加(会社の認証基盤自体がちゃんと作られ運用されている前提。基盤側が穴だらけなら乗っても守れません) ②記事末尾に🎁特典を新設——使いたいAIの情報を【】に埋めると「情シスから聞かれそうな質問+答え方のヒント(3つの型対応)+一緒に考える項目」を洗い出すコピペ用プロンプト ③冒頭に特典予告を追加。図解3枚も挿入済み。確認1点: 特典の置き場所はまとめの後(記事最後)です。まとめの前が良ければ入れ替えます。' },
