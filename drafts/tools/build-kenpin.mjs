...
const NOTES = {
  ...
  '021-level-kitei': { status: 'new', note: '【新規・ガバナンス】「個人情報は入力禁止」の一行ルールをレベル規程に書き直す方法(ルールを書く側向け。014=入力する側との住み分けを冒頭で明示)。レベル表は持ち帰り可能なMarkdown表で掲載。確認3点: ①失敗談に「消耗すると一行ルールに流れたくなる誘惑を知っている」という心情解釈を足しています ②Q4「監査に強いのは、実は線を引いたほう」の断定の強さ ③タイトルの「同じ箱に入れない」メタファー(本文でも繰り返し使用)。' },
