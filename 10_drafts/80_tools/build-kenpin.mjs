...
const NOTES = {
  ...
  '008-shanai-system': { status: 'fix', note: '🆕8/19: 新カード方式（ルール列挙ゼロ・カード6枚+見本だけで執筆）の試作1本目として再々構成しました。195行→135行。想定問答なし（新裁定どおり）。旧構成の「プロジェクト/スキルに置く」「手順書ごと作らせる」「文言だけ打ち込む逃げ道」は削除。⚠️失敗談なし（材料に本人の実話が無いため作らず。「分類1」実演の直後が差し込み適所です）。Copilot実物スクショは画面収録の許可待ち。旧方式との読み比べをお願いします。' },
  '057-nemawashi-junban': { status: 'new', note: '🆕8/19: 新カード方式の試作2本目（完全新規）。8/14聞き取りQ5が素材: 予算部門に先に話を通した勝ち筋＋SSOたらい回し（部門名は一般化済み）。想定問答なし。⚠️確認2点: ①「お金のOKとNGを判断できるのはあの部門だけだった」は証言からの敷衍です。実感と違えば直します ②特典（話を通す順番を洗い出すプロンプト）を付けました。不要なら外します。' },
  ...
  '021-level-kitei': { status: 'fix', note: '⚙️8/18「全部書き直し」指示: v4(PASONA型)で全面再構成+AI臭除去+Codex検品済み。約2割短縮。表と本文の整合(不可2行・セルは可/不可)維持。失敗談(EUパブコメ消耗)を共感位置へ、想定問答を3問に圧縮。' },
  '022-ai-browser': { status: 'fix', note: '⚙️8/18「全部書き直し」指示: v4(PASONA型)で全面再構成+AI臭除去+Codex検品済み。特典プロンプト・料金表(E8)は不変。失敗談は「登録3つが脱落ポイント」どまり。⚠️あなた自身が登録で詰まった実話があれば足すと強くなります。' },
