...
const NOTES = {
  ...
  'page-kousei-minaoshi': { status: 'new', note: '【新設③・未公開】サイト構成見直し・モック承認版(8/20)。トップは文言そのまま・カード4枚化(記事/カイゼン/AIガバナンス構築/AI法規制情報)+学び3カテゴリ10棚+/regulation/新設+新カード画像生成済み。確認3点: ①公開GO/NG ②014を「使い方の型」棚に入れるか ③ISO連載の予告8枚を出すか' },
  ...
  '059-eu-toumeisei': { status: 'ok', note: '✅8/19本人検品OK(スタンス節削除で確定)。図解1点+OGP生成中→公開準備。' },
  '060-kinshi-ai': { status: 'ok', note: '✅8/19本人検品OK。認識確認に「あなたが正しい」→descriptionの「使ってはいけない」も「禁止されている」に統一しました(冒頭と今日の一歩の話し言葉「使っちゃいけない/使ってはいけない」は読者のセリフなのでそのまま)。図解2点+OGP生成中→公開準備。' },
  '061-jibun-data': { status: 'ok', note: '✅8/19本人検品OK(2回目加筆=特徴リスト拡充+「AIにひとつのmdファイルにしてもらいましょう」を回収済み)。メモ: 確認1の「仕事でよく出るテーマ」の言及は、リストから同項目が消えた後も本人文のまま残しています(気になれば一言ください)。図解2点+OGP生成中→公開準備。' },
};
