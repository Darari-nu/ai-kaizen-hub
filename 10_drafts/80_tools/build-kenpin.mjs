...
const NOTES = {
  ...
  '008-shanai-system': { status: 'fix', note: '⚙️8/17検品反映・前半を全面書き直し: ①「歴戦の勇士」→いただいた言い回し「使いにくいことが多いです……。（作ってる方、すみません）」に ②結論=「AIのブラウザ拡張機能」を冒頭で言い切り→Copilotボタン→ChatGPT/Claude拡張(入れるなら会社の規則に従って)→ピンとこなければマニュアルをアップロードで完璧、の流れに再構成。スクショは「ちょっとたいへん」な代替手段に降格し、最後に「通用するのは社内システムがブラウザで動く場合限定」を明記 ③ツェッテルカステンにエビデンス追記: Anthropicの実験で、文書をブロック分割+文脈つきにすると検索失敗49%減(出典リンクつき)。確認1点: Copilotボタンの画像はこちらにEdge環境がなく実物が撮れません(拾い画像は権利面が怪しいので回避)。会社PCで右上のCopilotマーク付近のスクショを1枚いただければ差し込みます。' },
  '011-dare-ga-itta': { status: 'ok', note: '✅8/13検品OK「修正なし」(図解3枚込み)。ありがとうございます！' },
  '012-kuni-mo-yare': { status: 'fix', note: '⚙️8/17指示で解説記事に転換: 「会社は動かなかった」失敗談・ポジション取り論をすべて削除し、人工知能基本計画の現場語訳に一本化しました。第Ⅱ期(2026年7月14日閣議決定「日本AX、より強く、より豊かに」)を内閣府の概要版PDFで裏取りし、4方針(使う・創る・信頼を高める・協働する)+会社員に効く3点(様子見こそリスク/AXが公式語に/学び直しは国策と同じ向き)の構成に。タイトルも「『国もAIやれって言ってるの？』——言ってます。人工知能基本計画を現場語訳します」へ変更。確認1点: 8/21(金)公開予定はこのまま維持でよいですか。' },

  // --- 8/8 新規5本（Fableサブエージェントが1本ずつ執筆） ---
  '014-nyuuryoku-handan': { status: 'ok', note: '✅8/13検品OK「いいかな！」。ありがとうございます！' },
  '015-joushisu-shitsumon': { status: 'fix', note: '⚙️8/17検品反映: ①失敗談ブロックを丸ごと削除 ②Q1の「いいです。むしろ喜ばれます。」を削除し、回答を中立トーンで書き直し ③イヤミっぽい箇所を修正——「メールで動かないものは、会議体で動く。JTCの習性です」→「文面の往復で数週間かかる話が、顔を合わせた30分で片づくことはよくあります」/「ただの丸投げです」→「先にこちらで調べておく側の仕事です」。イヤミ・皮肉トーン禁止はRULES.md F2に鉄則登録し、今後の全記事に効かせます。🎁特典プロンプト(記事末尾)の確認は引き続きお待ちしています。' },
