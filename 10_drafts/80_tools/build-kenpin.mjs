...
const NOTES = {
  ...
  '012-kuni-mo-yare': { status: 'fix', note: '⚙️8/18検品反映: ①タイトル「DXの次はAX？会社に降りてくる「人工知能基本計画」を現場語訳します」(AX入り・——なし) ②結論ファースト: 「補助金・支援策・業界ガイドラインの形で会社に降りてきますよ」を冒頭5段落目に昇格 ③AI臭のメタ前置き(「効くところだけ置いていきます」「事実だけ並べます」「言ってます。それも〜」等)を全て削除 ④図解コメント3箇所(降りてくる流れ図/年表/4方針の田の字)を設置——文章OK後に生成します。事実・日付・出典は8/17版のまま。確認1点: 8/21(金)公開予定はこのまま維持でよいですか。' },

  // --- 8/8 新規5本（Fableサブエージェントが1本ずつ執筆） ---
  '014-nyuuryoku-handan': { status: 'ok', note: '✅8/13検品OK「いいかな！」。ありがとうございます！' },
  '015-joushisu-shitsumon': { status: 'fix', note: '⚙️8/18検品反映: ①「7名分申請して、通しました」→「7名分申請しました」(descriptionの「通した当事者」も「申請した当事者」に) ②タイトルの——を「。」に ③段落分割を5箇所追加して改行多め ④図解を2箇所追加——「情シスが守りたい3つ」の図(そもそも節)と「3ステップ全体の流れ図」(やってみた節直下)。発注コメントを設置済みで、文章OK後に生成します ⑤特典プロンプトに「# 役割」見出しを追加し構造化。🎁特典プロンプトの中身確認は引き続きお待ちしています。' },
  '016-excel-shukei': { status: 'fix', note: '⚙️8/18ご指定のストーリーで全面書き直し: ビフォー=ひとり1台1Excel→数式入りまとめExcel→月次PPT手転記(転記が面倒/稼働率悪い時の各装置Excel行脚が面倒)→アフター=アプリ化・報告書ボタンでPPT・人はコメントだけ。タイトル「まとめExcelからPPTへ、毎月転記してません？その報告書、ボタンひとつになります」。ポータルにビフォー実物を新設: 様式S-3 設備稼働月報まとめExcel(装置別3シート+実数式のまとめシート・印鑑欄・「PPTへ転記すること」の赤字注記つき。集計値はイントラ7月度確定値と一致)。イントラ通達にも「様式S-3提出廃止(アプリ移行)」を追加。確認2点: ①失敗談は旧記事の「検算せずに出しかけた」を流用しました。実話として合っていますか ②アプリの作り方は書いていません(指定ストーリーに無いため)。実話で書ける材料があれば教えてください。' },
  '017-gijiroku': { status: 'fix', note: '⚙️8/18検品反映: ①「内職をするな」失敗談を今度こそ丸ごと削除しました(前回は圧縮存置のご相談にしていました。すみません。復活禁止でルール登録済み) ②紙メモ前提の記述を全部PCメモに修正(「ぼくはPCでメモを取ります」) ③Q2に「ZoomやTeamsなら標準の録音機能。オンライン会議ならボタンひとつ」を追記 ④タイトルの——を「。」に ⑤図解2箇所追加(役割分担図/PCメモ×AI議事録の照合フロー)——コメント設置済み、文章OK後に生成。' },
  '020-hp-seisaku': { status: 'fix', note: '⚙️8/18ご指摘の修正1点: 「GitHubが公開までやってくれる」→「コード置き場はGitHub、公開はCloudflare」に訂正しました(darari-nu.comの配信がCloudflareなのを確認済み)。あわせてタイトルの——を「。」に。それ以外は8/13検品OK版のままです。公開前に訂正箇所だけ目視ください。' },
